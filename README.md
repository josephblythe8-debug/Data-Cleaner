...
This asserts the text and spreadsheet parsers both correctly extract
product names/prices (skipping headers, greetings, blank lines) and that
the matcher matches real garments — even reworded or reordered — while
...
| `validation.ts` | Blocking errors (missing club code/range/style/colour, duplicate SKU) vs. non-blocking warnings (a garment with zero sizes selected, a product with no price set yet — pricing is optional and can be added anytime). |
| `csvExport.ts` | Renders the BigCommerce import CSV from the same generated products Preview shows; a product with no price yet exports with a blank Price column. |
...
2. **Garments** — tick which garments this club needs from the library
   ...
   sizes/prices wholesale, or **import a brief**: paste, or upload a file
   of, an email/product brief, and it's matched against the garment
   library automatically — anything that doesn't match is flagged for
...
3. **Configure Store** — for each garment, set a custom product name,
   ...
   size-by-size), and optionally set a price per age-group product — a
   price can always be added or changed later, it never blocks anything.
   Drag to reorder, duplicate, or remove.
4. **Preview & Export** — see every generated product/SKU exactly as it
   will be exported, with blocking-error and warning banners (a missing
   price is a warning, not a blocker — export whenever you're ready), then
   download the BigCommerce CSV.

### Importing a brief

The "Import Brief" button (Step 2) accepts pasted text, or an uploaded
file — **Excel (`.xlsx`)** is read natively (each row's cells by their
real type, so a plain price cell like `45` with no `$` or decimals is
still found reliably), as are `.csv`/`.tsv`/`.txt`. Any other file type
(`.pdf`, `.docx`, images, ...) isn't parsed directly yet — the dialog
says so and asks for the text to be pasted in instead, rather than
silently misreading it. Each line/row is parsed for a product name and an
optional price, matched against the garment library by name (tolerant of
reordering and age/gender qualifiers like "(Adults)"), and shown for
review: matched lines are pre-checked with an editable (optional) price,
